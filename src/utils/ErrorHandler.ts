...
export class ErrorHandler {
  static createError(type: ErrorTypes, message: string, status: number, details: any): AppError {
    return {
      type,
      status,
      message,
  ...
  static logError(error: AppError): void {
    console.error(`[${error.timestamp}] ${error.type}: ${error.message}`, error.details)
  }
  ...
  static mapAxiosErrorToAppError = (error: AxiosError): AppError => {
    ...
    switch (status) {
      case 400:
        ...
        return this.createError(ErrorTypes.VALIDATION_ERROR, message, status, details);
      case 401:
      ...
      case 403:
        ...
        return this.createError(ErrorTypes.FORBIDDEN_ERROR, message, status, details);
      case 404:
        // (No encontrado, recurso inexistente)
        return this.createError(ErrorTypes.NOT_FOUND_ERROR, message, status, details);
      case 500:
      default:
        // (Error interno del servidor o cualquier otro error no mapeado)
        return this.createError(ErrorTypes.SERVER_ERROR, message, status, details);
    }
  };
}